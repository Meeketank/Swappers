...
export default function Dashboard() {
    ...
    const fetchUsers = async () => {
      try {
        ...
        if (!currentUserDoc.empty) {
          const userData = currentUserDoc.docs[0].data() as UserProfile;
          const currentUserWithId = {
            ...userData,
            id: currentUserDoc.docs[0].id
          };
          setCurrentUser(currentUserWithId);
          ...
          // Filter matched profiles (users who have skills current user wants)
          const matched = profiles.filter(profile => 
            profile.skillsOffered.some(skill => 
              currentUserWithId.skillsWanted.includes(skill)
          ));

          // Other profiles are those that don't match skills
  ...
  useEffect(() => {
    ...
    // Update matched and other profiles based on search
    if (currentUser) {
      const matched = filtered.filter(profile => 
        profile.skillsOffered.some(skill => 
          currentUser.skillsWanted.includes(skill))
      );
      const others = filtered.filter(profile => 
        !matched.some(mp => mp.id === profile.id)
      );
      setMatchedProfiles(matched);
      setOtherProfiles(others);
    ...
    return (
      <div className="flex justify-center items-center h-screen bg-gradient-to-br from-indigo-50 to-blue-100">
        <div className="text-center">
          ...
          <p className="mt-4 text-lg font-medium text-gray-700">Loading your skill swap matches...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="min-h-screen bg-gradient-to-br from-indigo-50 to-blue-100">
        ...
        <main className="max-w-7xl mx-auto px-4 py-6 sm:px-6 lg:px-8">
          ...
          <div className="mb-12 text-center">
            ...
            {/* Enhanced Search Bar */}
            // Update the search bar component in your Dashboard
<div className="max-w-2xl mx-auto relative">
  ...
  {searchTerm && (
    <button
      onClick={() => {
        setSearchTerm('');
        setFilteredProfiles(allProfiles); // Reset to show all profiles
        // Recalculate matched and other profiles
        if (currentUser) {
        ...
        <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          {matchedProfiles.length > 0 ? (
            ...
            <div className="bg-white rounded-xl shadow-md p-8 text-center mb-12">
              ...
              <p className="text-gray-500">We couldn't find any swappers matching your search criteria</p>
            </div>
          ) : null}
          ...
          {/* Empty State */}
          {!matchedProfiles.length && !otherProfiles.length && !searchTerm && (
            ...
            <div className="bg-white rounded-xl shadow-xl max-w-md w-full max-h-[90vh] overflow-y-auto animate-fade-in">
              <div className="p-6">
                  ...
                  <div className="flex items-center">
                    <div className="w-20 h-20 rounded-full bg-gradient-to-br from-blue-100 to-purple-100 overflow-hidden mr-4 flex items-center justify-center">
                      {selectedProfile.profilePic ? (
                        <img src={selectedProfile.profilePic} alt={selectedProfile.firstName} className="w-full h-full object-cover" />
                      ) : (
                    ...
                    <div>
                      <h2 className="text-xl font-bold text-gray-800">{selectedProfile.firstName} {selectedProfile.lastName}</h2>
                      <div className="flex items-center">
                        {[...Array(5)].map((_, i) => (
                          <svg
                            key={i}
                            className={`h-5 w-5 ${i < Math.floor(selectedProfile.rating) ? 'text-yellow-400' : 'text-gray-300'}`}
                            fill="currentColor"
                            viewBox="0 0 20 20"
                          >
                        ...
                        {(selectedProfile.rating || 0).toFixed(1)}
                        </span>
                      </div>
                    </div>
                  </div>
                ...
                <div className="mb-6">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {selectedProfile.skillsWanted.map((skill) => (
                ...
                {connections.some(conn => 
                  conn.user1Id === auth.currentUser?.uid && conn.user2Id === selectedProfile.id ||
                  conn.user2Id === auth.currentUser?.uid && conn.user1Id === selectedProfile.id
                ) ? (
                  <button
                    disabled
                  ...
                  <button
                    onClick={() => {
                      handleSendRequest(selectedProfile.id);
                      setSelectedProfile(null);
                    }}
...
const ProfileCard = ({ profile, onSelect, onSendRequest, isMatch, connections }: {
  profile: UserProfile;
      ...
      <div className="p-6">
        <div className="flex items-center mb-4">
          <div className={`w-16 h-16 rounded-full overflow-hidden mr-4 flex items-center justify-center ${!profile.profilePic ? (isMatch ? 'bg-blue-100' : 'bg-purple-100') : ''}`}>
            {profile.profilePic ? (
              <img src={profile.profilePic} alt={profile.firstName} className="w-full h-full object-cover" />
            ) : (
        ...
        {isConnected ? (
          ...
          <button
            ...
            className={`w-full mt-4 py-2 px-4 rounded-lg font-medium transition-all duration-300 ${isMatch ? 
